...
4) Винести спільну логіку в `base.BaseTest`.
...
- Якщо використовуєте паралельність, краще, щоб `base.BaseTest` створював окремий інстанс драйвера на клас або на метод (за потреби можна перейти на `@BeforeMethod`).
...
- Є `base.BaseTest` із ініціалізацією та завершенням WebDriver, а також налаштованими `implicit` та/або `explicit` wait-ами.
...
- Не дублюйте код: спільне — у `base.BaseTest` або утилітарних класах.
